...
char *hrest_vc_id = "$Id: HLlike.c,v 1.2 2015/06/19 21:15:23 tuc56736 Exp $";
...
/*
These arrya are defined to save the start and end time in input segment file
 */
long *st_time;
long *en_time;
...
void LoadFile(char *fn)
{
   ...
   st_time=(long*)malloc(nObs);
   en_time=(long*)malloc(nObs);
   ...
   for (seg=1;seg<=nSeg;seg++) {
	...
	T=SegLength(segStore,seg);
	 ...
	 if (1==1){ /* above line cause some examples to  be escaped */
	    ...
	    segProb = ap;
	    fprintf(fp,"%ld\t%ld\t%s\t%f\n",st_time[seg-1],en_time[seg-1],segLab,segProb);
